...
3. **Rename `apply_line_edits` to `edit_lines`**:
   ...
   - If called directly on a file without an active session (e.g. for `prepend`/`append` operations), `edit_lines` will automatically call the private `init_edit_session` function in the background.
...
- **Descriptions**:
  ...
  - `edit_lines`: `"Applies a structured batch of line edits (insert_after, insert_before, append, prepend, update, delete, replace_range, move) transactionally to any text file. Performs syntax validation for supported programming languages."`
